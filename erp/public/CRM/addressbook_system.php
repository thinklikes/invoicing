...
if($_SESSION['MM_Username']!=""){
	...
	{
		$sql_de = "delete from addressbook where no = '".$_GET["deliid"]."'";
...
if (isset($_GET["s"]) and !empty($_GET["s"])) {
	$status=" status='".$_GET["s"]."' and ";
}
else{
	$status="";
}
$query_RecMember = sprintf("SELECT * FROM addressbook WHERE  $status %s LIKE %s ORDER BY no", $colname_RecMember,GetSQLValueString($keyword_RecMember, "text"));
...
<div id="wrapper">
        ...
        <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
            ...
            <div class="navbar-header">
                ...
                <a class="navbar-brand" href="addressbook_system.php"><?php echo $sys_name; ?></a>
            </div>
            <!-- Top Menu Items -->
        ...
        <div id="page-wrapper">
         ...
         <div class="search">
           <form name="form1" method="get" action="addressbook_system.php"  class="form-inline" >
<strong>
               <select name="colname" id="colname" class="btn btn-primary dropdown-toggle">
			    <!--<select name="search" id="search" class="btn btn-primary dropdown-toggle">-->
					<option value="name" selected>查詢名稱</option>
					<option value="tel_1">查詢電話一</option>
					<option value="tel_2">查詢電話二</option>
					<option value="phone">查詢手機</option>
					<option value="fax">查詢傳真</option>
               </select>
               </strong>
             ...
             <input name="button2" type="button" id="button2" class="btn btn-default" onClick="MM_goToURL('parent','addressbook_system.php');return document.MM_returnValue" value="顯示所有資料">
          
		   </form>
        </div><BR>
	    <div class="btn-group">
		<div class="container-fluid">
            <button type="button" class="btn btn-info" onclick="window.open('addressbook_add.php', 'newwindow', 'height=700, width=800');">新增通訊錄</button>
       </div><br>
	   <div class="container-fluid">
            <ul class="nav nav-tabs">
				...
				?>
			  <li <?php echo $tmp;?>><a href="addressbook_system.php">全部</a></li>
			  <li <?php echo $tmp1;?>><a href="addressbook_system.php?s=1">傑瑀</a></li>
			  <li <?php echo $tmp2;?>><a href="addressbook_system.php?s=2">高雄大容</a></li>
			  <li <?php echo $tmp3;?>><a href="addressbook_system.php?s=3">台北大容</a></li>
			  <li <?php echo $tmp4;?>><a href="addressbook_system.php?s=4">台中大容</a></li>
			  <li <?php echo $tmp5;?>><a href="addressbook_system.php?s=5">國群</a></li>
			  <li <?php echo $tmp6;?>><a href="addressbook_system.php?s=6">國貿局</a></li>
			  <li <?php echo $tmp7;?>><a href="addressbook_system.php?s=7">網路公司</a></li>
			  <li <?php echo $tmp8;?>><a href="addressbook_system.php?s=8">公會</a></li>
			  <li <?php echo $tmp9;?>><a href="addressbook_system.php?s=9">檢驗局</a></li>
			</ul>
       </div><BR>
     </div>	 <BR>
  ...
  <!--<table class="table table-striped table_01">-->
    ...
    <tbody>
      <tr class="td_row">
			<td data-th="名稱"><?php echo $row_RecMember['name'];?></td>
			<td data-th="電話一"><?php echo $row_RecMember['tel_1'];?></td>
			<td data-th="電話二"><?php echo $row_RecMember['tel_2'];?></td>
			<td data-th="手機"><?php echo $row_RecMember['phone'];?></td>
			<td data-th="傳真"><?php echo $row_RecMember['fax'];?></td>
			<td data-th="備註"><?php echo $row_RecMember['remark'];?></td>
			<td data-th="功能">
				<input type="button" value="修改" class="btn btn-default" onclick="window.open('addressbook_system_edit.php?nid=<?PHP echo $auto_id;?>', 'newwindow', 'height=450, width=550, top=50, left=200, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no');">
				<input type="button" value="刪除" class="btn btn-default" onclick="if(confirm('確定要刪除這一筆資料?'))location.href='addressbook_system.php?deliid=<?php echo $auto_id;?>';">
			</td>
        </tr>
