...
if($_SESSION['MM_Username']!=""){
	...
	{
		$sql_de = "delete from customer_service_2 where no = '".$_GET["deliid"]."'";
...
if (isset($_GET["s"]) and !empty($_GET["s"])) {
	$str=$_GET["s"]-1;
	$status=" quote='$str' and ";
}
else{
	$status="";
}
$query_RecMember = sprintf("SELECT * FROM customer_service_2 WHERE  $status %s LIKE %s ORDER BY no", $colname_RecMember,GetSQLValueString($keyword_RecMember, "text"));
...
<div id="wrapper">
        ...
        <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
            ...
            <div class="navbar-header">
                ...
                <a class="navbar-brand" href="customer_service_system.php"><?php echo $sys_name; ?></a>
            </div>
            <!-- Top Menu Items -->
        ...
        <div id="page-wrapper">
         ...
         <div class="search">
           <form name="form1" method="get" action="customer_service_system.php"  class="form-inline" >
<strong>
               <select name="colname" id="colname" class="btn btn-primary dropdown-toggle">
			    <!--<select name="search" id="search" class="btn btn-primary dropdown-toggle">-->
					<option value="in_date" selected>查詢日期</option>
					<option value="c_id">查詢公司名稱</option>
					<option value="quote">報價</option>
					<option value="operator">處理人</option>
               </select>
               </strong>
             ...
             <input name="button2" type="button" id="button2" class="btn btn-default" onClick="MM_goToURL('parent','customer_service_system.php');return document.MM_returnValue" value="顯示所有資料">
          
		   </form>
        </div><BR>
	    <div class="btn-group">
		<div class="container-fluid">
            <button type="button" class="btn btn-info" onclick="window.open('customer_service_add.php', 'newwindow', 'height=700, width=800');">新增內容</button>
       </div><br>
	   <div class="container-fluid">
            <ul class="nav nav-tabs">
				...
				?>
			  <li <?php echo $tmp;?>><a href="customer_service_system.php">全部</a></li>
			  <li <?php echo $tmp1;?>><a href="customer_service_system.php?s=1">客戶服務</a></li>
			  <li <?php echo $tmp2;?>><a href="customer_service_system.php?s=2">報價</a></li>
			</ul>
       </div><BR>
     </div>	 <BR>
  ...
  <!--<table class="table table-striped table_01">-->
    ...
    <tbody>
      <tr class="td_row">
			<td data-th="日期"><?php echo $row_RecMember['in_date']." ".$row_RecMember['in_time'];?></td>
			<?php
			...
			<td data-th="反應內容"><?php echo $row_RecMember['content'];?></td>
			<?php if($status!=1){ ?>
			<td data-th="報價"><?php if($row_RecMember['quote']==1)echo "是";?></td>
			<td data-th="報價內容"><?php echo $row_RecMember['quote_contact'];?></td>
			<?php } ?>
			<td data-th="費用"><?php echo $row_RecMember['cost'];?></td>
			<td data-th="備註"><?php echo $row_RecMember['remark'];?></td>
			<td data-th="處理人"><?php echo $row_RecMember['operator'];?></td>
			<td data-th="功能">
				<input type="button" value="修改" class="btn btn-default" onclick="window.open('customer_service_system_edit.php?nid=<?PHP echo $auto_id;?>', 'newwindow', 'height=450, width=550, top=50, left=200, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no');">
				<input type="button" value="刪除" class="btn btn-default" onclick="if(confirm('確定要刪除這一筆資料?'))location.href='customer_service_system.php?deliid=<?php echo $auto_id;?>';">
			</td>
        </tr>
