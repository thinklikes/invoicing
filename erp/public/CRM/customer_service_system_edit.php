...
if (isset($_GET["nid"])) {
  $auto_id=$_GET["nid"];
  $LoginRS__query = sprintf("SELECT * FROM customer_service_2 WHERE no=%s", GetSQLValueString($auto_id, "text"));
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
	$year=date("Y")-1911;
	$last_date=$year.date("md");
	
  $updateSQL = sprintf("update customer_service_2 set `c_id`=%s, `content`=%s, `quote`=%s, `quote_contact`=%s, `cost`=%s, `remark`=%s, `last_operator`=%s, `in_date`=%s, `in_time`=%s,last_date=%s,last_time=%s where no=%s",
					   ...
					   GetSQLValueString($last_date, "text"),
          ...
          <div class="panel-body">
                      ...
                      <form class="form-horizontal" role="form" action="<?php echo $editFormAction; ?>" METHOD="POST" name="form1" id="form1" onSubmit="return dataCheck()">
									<div class="form-group">
										 ...
										 <?php
											$yyy=substr($result["in_date"],0,3);
											$mm=substr($result["in_date"],3,2);
											$dd=substr($result["in_date"],5,2);
										 ?>
                 ...
                 <textarea name="content" rows="4" cols="50" class="form-control" id="content"><?php echo $result["content"]; ?></textarea>
                 </span><br>
										 </div>
                                    </div>
                 ...
                 <input type="checkbox" name="quote" id="quote"  value="1"<?php if($result["quote"]==1) echo "checked "; ?> onclick="show(this, 'hiddenbox')">
                 </span><br>
										 </div>
                                    </div>
									
									<div class="form-group" id="hiddenbox" <?php if($result["quote"]==0) echo 'style="display:none;"'; ?>>
                                         <label  class="col-sm-2 control-label">報價內容</label>
                                         <div class="col-sm-10"><span id="sprytextfield4">
                 <textarea name="quote_contact" rows="4" cols="50" class="form-control" id="quote_contact"><?php echo $result["quote_contact"]; ?></textarea>
                 </span><br>
										 </div>
                                    </div>
