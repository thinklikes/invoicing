...
if($_SESSION['MM_Username']!=""){
	...
	{
		$sql_de = "delete from das where no = '".$_GET["deliid"]."'";
...
$query_RecMember = sprintf("SELECT * FROM das WHERE %s LIKE %s ORDER BY no", $colname_RecMember,GetSQLValueString($keyword_RecMember, "text"));
...
<div id="wrapper">
        ...
        <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
            ...
            <div class="navbar-header">
                ...
                <a class="navbar-brand" href="das_system.php"><?php echo $sys_name; ?></a>
            </div>
            <!-- Top Menu Items -->
        ...
        <div id="page-wrapper">
         ...
         <div class="search">
           <form name="form1" method="get" action="das_system.php"  class="form-inline" >
<strong>
             ...
             <input name="button2" type="button" id="button2" class="btn btn-default" onClick="MM_goToURL('parent','das_system.php');return document.MM_returnValue" value="顯示所有資料">
          
		   </form>
        </div><BR>
	    <div class="btn-group">
		<div class="container-fluid">
            <button type="button" class="btn btn-info" onclick="window.open('das_system_add.php', 'newwindow', 'height=700, width=800');">新增</button>
       </div><br>
	   <div class="container-fluid">
  ...
  <?php $i=1; do {  $auto_id=$row_RecMember['no'];?>
	  ...
	  <tr class="td_row" style="background-color:#CCEEFF">
			...
			<td data-th="合約期間"><?php echo $row_RecMember['contract_period'];?></td>
			<td data-th="購買日期"><?php echo $row_RecMember['contract_buy'];?></td>
			<td data-th="百年蟲升級費" ><?php echo $row_RecMember['up_money'];?></td>
			<td data-th="百年蟲升級費備註"><?php echo $row_RecMember['up_remark'];?></td>
	  </tr>	  
	  <tr class="th_row" style="background-color:#5599FF;">
		<th>買賣合約</th>
		<th>付費修改報單號碼</th>
	  ...
	  <tr class="td_row" style="background-color:#CCEEFF">     
			<td data-th="買賣合約"><?php  if($row_RecMember['contract_YN']==1)echo "V";?></td>
			<td data-th="付費修改報單號碼"><?php echo $row_RecMember['declaration_no'];?></td>
			<td data-th="DAS主機位置"><?php echo $row_RecMember['server_address'];?></td>
			<td data-th="DAS台數"><?php echo $row_RecMember['server_amount'];?></td>
			<td data-th="備註"><?php echo $row_RecMember['remark'];?></td>
			<td data-th="功能" colspan="2">
				<input type="button" value="修改" class="btn btn-default" onclick="window.open('das_system_edit.php?nid=<?PHP echo $auto_id;?>', 'newwindow', 'height=450, width=550, top=50, left=200, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no');">
				<input type="button" value="刪除" class="btn btn-default" onclick="if(confirm('確定要刪除這一筆資料?'))location.href='das_system.php?deliid=<?php echo $auto_id;?>';">
			</td>
        </tr>
