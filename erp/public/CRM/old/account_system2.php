...
if($_SESSION['MM_Username']!=""){
	...
	{
		$sql_de = "delete from account_db2 where account_id = ".$_GET["deliid"]."";
		//echo $sql;
...
$query_RecMember = sprintf("SELECT * FROM account_db2 WHERE  %s LIKE %s ORDER BY account_id", $colname_RecMember,GetSQLValueString($keyword_RecMember, "text"));
...
/*
mysql_select_db($database_connSQL, $connSQL);
$query_RecUser = sprintf("SELECT * FROM  account_db2 WHERE donation_name = %s", GetSQLValueString($colname_RecUser, "text"));
...
<div id="wrapper">
        ...
        <div id="page-wrapper">
         ...
         <div class="search">
           <form name="form1" method="get" action="account_system2.php"  class="form-inline" >
<strong>
             ...
             <input name="button2" type="button" id="button2" class="btn btn-default" onClick="MM_goToURL('parent','account_system2.php');return document.MM_returnValue" value="顯示所有資料">
          
		   </form>
         </div><BR>
		 <!-- ***********************************第二個搜尋***************************-->
		 <div class="search">
           <form name="form2" method="get" action="account_report2.php"  class="form-inline" >
			<strong>
				<input type="text" class="form-control" name="p_keyword" id="p_keyword">     
                <select name="search" id="search" class="btn btn-primary dropdown-toggle">
		 ...
		 <div class="container-fluid">
			<?php	
				$rs8 =  mysqli_query($connSQL,"SELECT SUM(account_income) as account_income FROM account_db2  ");
				$row8=mysqli_fetch_array($rs8);
				$account_income=$row8["account_income"];

				$rs4 =  mysqli_query($connSQL,"SELECT SUM(account_income) as account_income FROM account_db2 where account_subject= '銀行存款' ");	
				$row4=mysqli_fetch_array($rs4);
				$account_income2=$row4["account_income"];
				
				$rs3 =  mysqli_query($connSQL,"SELECT SUM(account_expenses) as account_expenses FROM account_db2 ");	
				$row3=mysqli_fetch_array($rs3);
				$account_expenses=$row3["account_expenses"];
				
				$rs9 =  mysqli_query($connSQL,"SELECT SUM(account_surplus) as account_surplus FROM account_db2  ");	
		 ...
		 <!-- ***********************************總計***************************-->
				<div class="container-fluid">
					<button type="button" class="btn btn-info" onclick="window.open('account_system_backup2.php');">匯出Excel</button>
					<button type="button" class="btn btn-info" onclick="window.open('account_add2.php', 'newwindow', 'height=800, width=700');">新增資訊</button>
				</div>
    ...
    <tbody>
      <tr class="td_row">
        ...
        <!--td><a href="pray_system.php?auto_id=<?php echo $row_RecMember['auto_id']; ?>">修改</a> <a href="pray_system.php?auto_id=<?php echo $row_RecMember['auto_id']; ?>&dele=true" onClick="tfm_confirmLink('確定要刪除這筆會員資料嗎？');return document.MM_returnValue">刪除</a></td-->
			<td data-th="功能">
			<input type="button" value="修改" class="btn btn-default" onclick="window.open('account_system_edit2.php?nid=<?PHP echo $auto_id;?>', 'newwindow', 'height=450, width=550, top=50, left=200, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no');">
			<input type="button" value="刪除" class="btn btn-default" onclick="if(confirm('確定要刪除這一筆資料?'))location.href='account_system2.php?deliid=<?php echo $auto_id;?>';">
		</td>
      </tr>
