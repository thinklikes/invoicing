...
if($_SESSION['MM_Username']!=""){
	...
	{
		$sql_de = "delete from donation_3_db where donation_id = ".$_GET["deliid"]."";
		//echo $sql;
...
$query_RecMember = sprintf("SELECT * FROM donation_3_db WHERE  %s LIKE %s ORDER BY donation_id", $colname_RecMember,GetSQLValueString($keyword_RecMember, "text"));
...
/*
mysql_select_db($database_connSQL, $connSQL);
$query_RecUser = sprintf("SELECT * FROM  donation_3_db WHERE donation_name = %s", GetSQLValueString($colname_RecUser, "text"));
...
<div id="wrapper">
        ...
        <div id="page-wrapper">
         ...
         <div class="search">
           <form name="form1" method="get" action="donation_3_system.php"  class="form-inline" >
<strong>
             ...
             <input name="button2" type="button" id="button2" class="btn btn-default" onClick="MM_goToURL('parent','donation_3_system.php');return document.MM_returnValue" value="顯示所有資料">
          
		   </form>
         </div><BR>
		 <!-- ***********************************第二個搜尋***************************-->
		 <div class="search">
           <form name="form2" method="get" action="donation_3_report.php"  class="form-inline" >
			<strong>
				<input type="text" class="form-control" name="p_keyword" id="p_keyword">     
                <select name="search" id="search" class="btn btn-primary dropdown-toggle">
		 ...
		 <div class="container-fluid">
			<?php	
				$total = mysqli_num_rows(mysqli_query($connSQL,"select * from donation_3_db"));
				$rs2 =  mysqli_query($connSQL,"SELECT SUM(donation_money) FROM donation_3_db ");	
		 ...
		 <!-- ***********************************總計***************************-->
				<div class="container-fluid">
					<button type="button" class="btn btn-info" onclick="window.open('donation_3_system_backup.php');">匯出Excel</button>
					<button type="button" class="btn btn-info" onclick="window.open('donation_3_add.php', 'newwindow', 'height=800, width=700');">新增資訊</button>
				</div>
  ...
  <table class="table table-striped table_01">
    ...
    <tbody>
      <tr class="td_row">
			...
			<td data-th="住址"><?php echo $row_RecMember['donation_address'];?><!--名稱--></td>
			<td data-th="品項"><?php echo $row_RecMember['donation_item'];?><!--聯絡人--></td>
			<td data-th="備註"><?php echo $row_RecMember['donation_remark'];?><!--手機--></td>
        <!--td><a href="pray_system.php?auto_id=<?php echo $row_RecMember['auto_id']; ?>">修改</a> <a href="pray_system.php?auto_id=<?php echo $row_RecMember['auto_id']; ?>&dele=true" onClick="tfm_confirmLink('確定要刪除這筆會員資料嗎？');return document.MM_returnValue">刪除</a></td-->
			<td data-th="功能">
			<input type="button" value="修改" class="btn btn-default" onclick="window.open('donatoin_3_system_edit.php?nid=<?PHP echo $auto_id;?>', 'newwindow', 'height=450, width=550, top=50, left=200, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no');">
			<input type="button" value="感謝狀" class="btn btn-default" onclick="window.open('donation_3_gratitude.php?nid=<?PHP echo $auto_id;?>', 'newwindow');">
			<input type="button" value="刪除" class="btn btn-default" onclick="if(confirm('確定要刪除這一筆資料?'))location.href='donation_3_system.php?deliid=<?php echo $auto_id;?>';">
		</td>
      </tr>
