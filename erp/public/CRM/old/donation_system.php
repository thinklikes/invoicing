...
if($_SESSION['MM_Username']!=""){
	...
	{
		$sql_de = "delete from donation_db where donation_id = ".$_GET["deliid"]."";
		//echo $sql;
...
$colname_RecMember = "donation_name";
...
$query_RecMember = sprintf("SELECT * FROM donation_db WHERE  %s LIKE %s ORDER BY donation_id", $colname_RecMember,GetSQLValueString($keyword_RecMember, "text"));
$query_limit_RecMember = sprintf("%s LIMIT %d, %d", $query_RecMember, $startRow_RecMember, $maxRows_RecMember);
$RecMember = mysqli_query($connSQL,$query_limit_RecMember) or die(mysqli_error());
...
/*
mysql_select_db($database_connSQL, $connSQL);
$query_RecUser = sprintf("SELECT * FROM  donation_db WHERE doantion_name = %s", GetSQLValueString($colname_RecUser, "text"));
...
<div id="wrapper">
        ...
        <div id="page-wrapper">
         ...
         <div class="search">
           <form name="form1" method="get" action="donation_system.php"  class="form-inline" >
<strong>
               <select name="colname" id="colname" class="btn btn-primary dropdown-toggle">
			    <!--<select name="search" id="search" class="btn btn-primary dropdown-toggle">-->
					<option value="donation_name" selected>查詢姓名</option>	
					<option value="donation_year">查詢年份</option>
					<option value="donation_address">查詢地址</option>
					<option value="donation_name2">查詢緣首</option>
               </select>
               </strong>
             ...
             <input name="button2" type="button" id="button2" class="btn btn-default" onClick="MM_goToURL('parent','donation_system.php');return document.MM_returnValue" value="顯示所有資料">
          
		   </form>
         </div><BR>
		 <!-- ***********************************第二個搜尋***************************-->
		 <div class="search">
           <form name="form2" method="get" action="donation_report.php"  class="form-inline" >
			<strong>
				<input type="text" class="form-control" name="p_keyword" id="p_keyword">     
                <select name="search" id="search" class="btn btn-primary dropdown-toggle">
					<option value="donation_year" selected>年/月/日報表</option>	
					<option value="donation_name">姓名</option>
					<option value="donation_address">住址</option>
					<option value="donation_name2">緣首</option>
					<option value="donation_item">品項</option>
					<option value="donation_money">捐獻金額</option>
					<option value="handled">經手人</option>
					<option value="donation_remark">備註</option>
                </select>
                </strong>        
		 ...
		 <div class="container-fluid">
			<?php	
				$total = mysqli_num_rows(mysqli_query($connSQL,"select * from donation_db "));
				$rs2 =  mysqli_query($connSQL,"SELECT SUM(donation_money) FROM donation_db ");	
					list($donation_money) = mysqli_fetch_row($rs2)				
				?>
				總計捐獻人數為：<?php echo $total;?> 人  &nbsp&nbsp&nbsp&nbsp&nbsp <?php echo' 總計捐獻金額為：$ ', number_format($donation_money);?>
         </div><BR>
		 <!-- ***********************************總計***************************-->
		 
		 <div class="container-fluid">
				<button type="button" class="btn btn-info" onclick="window.open('donation_system_backup2.php');">匯出緣首</button>
				<button type="button" class="btn btn-info" onclick="window.open('donation_system_backup.php');">匯出Excel</button>
				<button type="button" class="btn btn-info" onclick="window.open('donation_add.php', 'newwindow', 'height=800, width=700');">新增資訊</button>
  ...
  <table class="table table-striped table_01">
	...
	<?php do { $auto_id=$row_RecMember['donation_id']; ?>
    <tbody>
      <tr class="td_row">
			<td data-th="捐獻日期"><?php echo $row_RecMember['donation_year'];?><!--名稱--></td>
			<td data-th="姓名"><?php echo $row_RecMember['donation_name'];?><!--名稱--></td>
			<td data-th="緣首"><?php echo $row_RecMember['donation_name2'];?><!--名稱--></td>
			<td data-th="住址"><?php echo $row_RecMember['donation_address'];?><!--聯絡人--></td>
			<td data-th="捐獻金額"><?php echo $row_RecMember['donation_money'];?><!--手機--></td>
        <!--td><a href="pray_system.php?auto_id=<?php echo $row_RecMember['auto_id']; ?>">修改</a> <a href="pray_system.php?auto_id=<?php echo $row_RecMember['auto_id']; ?>&dele=true" onClick="tfm_confirmLink('確定要刪除這筆會員資料嗎？');return document.MM_returnValue">刪除</a></td-->
			<td data-th="功能">
			<input type="button" value="修改" class="btn btn-default" onclick="window.open('donatoin_system_edit.php?nid=<?PHP echo $auto_id;?>', 'newwindow', 'height=450, width=550, top=50, left=200, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no');">
			<input type="button" value="感謝狀" class="btn btn-default" onclick="window.open('donation_gratitude.php?nid=<?PHP echo $auto_id;?>', 'newwindow');">
			<input type="button" value="刪除" class="btn btn-default" onclick="if(confirm('確定要刪除這一筆資料?'))location.href='donation_system.php?deliid=<?php echo $auto_id;?>';">
		</td>
      </tr>
