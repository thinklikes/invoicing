...
if($_SESSION['MM_Username']!=""){
	...
	{
		$sql_de = "delete from update_kao where no = '".$_GET["deliid"]."'";
...
$query_RecMember = sprintf("SELECT * FROM update_kao WHERE %s LIKE %s ORDER BY no", $colname_RecMember,GetSQLValueString($keyword_RecMember, "text"));
...
<div id="wrapper">
        ...
        <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
            ...
            <div class="navbar-header">
                ...
                <a class="navbar-brand" href="update_kao_system.php"><?php echo $sys_name; ?></a>
            </div>
            <!-- Top Menu Items -->
        ...
        <div id="page-wrapper">
         ...
         <div class="search">
           <form name="form1" method="get" action="update_kao_system.php"  class="form-inline" >
<strong>
             ...
             <input name="button2" type="button" id="button2" class="btn btn-default" onClick="MM_goToURL('parent','update_kao_system.php');return document.MM_returnValue" value="顯示所有資料">
          
		   </form>
        </div><BR>
	    <div class="btn-group">
		<div class="container-fluid">
            <button type="button" class="btn btn-info" onclick="window.open('update_kao_add.php', 'newwindow', 'height=700, width=800');">新增</button>
       </div><br>
	   <div class="container-fluid">
  ...
  <?php $i=1; do {  $auto_id=$row_RecMember['no'];?>
	  ...
	  <tr class="td_row" style="background-color:#CCEEFF">
			...
			<td data-th="版本備註"><?php echo $row_RecMember['version_remark'];?></td>
			<td data-th="Memo"><?php 
			...
			elseif($memo[1]=="air")
				echo "空運";
			?></td>
			<td data-th="海運出口" ><?php  if($row_RecMember['ocean_out']==1)echo "V";?></td>
			<td data-th="海運進口"><?php  if($row_RecMember['ocean_in']==1)echo "V";?></td>
			<td data-th="空運出口"><?php  if($row_RecMember['air_out']==1)echo "V";?></td>
			<td data-th="空運進口"><?php  if($row_RecMember['air_out']==1)echo "V";?></td>
			<td data-th="轉運"><?php  if($row_RecMember['transport']==1)echo "V";?></td>
			<td data-th="X101"><?php  if($row_RecMember['X101']==1)echo "V";?></td>
			<td data-th="X201"><?php  if($row_RecMember['X201']==1)echo "V";?></td>
			<td data-th="X301"><?php  if($row_RecMember['X301']==1)echo "V";?></td>
	  </tr>	  
	  <tr class="th_row" style="background-color:#5599FF;">
		<th>X301_DN</th>
		<th>X401</th>
	  ...
	  <tr class="td_row" style="background-color:#CCEEFF">     
			<td data-th="X301_DN"><?php  if($row_RecMember['X301_DN']==1)echo "V";?></td>
			<td data-th="X401"><?php  if($row_RecMember['X401']==1)echo "V";?></td>
			<td data-th="X501"><?php  if($row_RecMember['X501']==1)echo "V";?></td>
			<td data-th="X601"><?php  if($row_RecMember['X601']==1)echo "V";?></td>
			<td data-th="X603"><?php  if($row_RecMember['X603']==1)echo "V";?></td>
			<td data-th="車單"><?php  if($row_RecMember['car']==1)echo "V";?></td>
			<td data-th="通知申請簽審憑證"><?php echo $row_RecMember['application'];?></td>
			<td data-th="簽審憑證測試日期雙向"><?php echo $row_RecMember['voucher_2_date'];?></td>
			<td data-th="簽審憑證測試日期三向"><?php echo $row_RecMember['voucher_3_date'];?></td>
			<td data-th="簽審憑證核准日期"><?php echo $row_RecMember['voucher_ok_date'];?></td>
			<td data-th="功能" colspan="2">
				<input type="button" value="修改" class="btn btn-default" onclick="window.open('update_kao_system_edit.php?nid=<?PHP echo $auto_id;?>', 'newwindow', 'height=450, width=550, top=50, left=200, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no');">
				<input type="button" value="刪除" class="btn btn-default" onclick="if(confirm('確定要刪除這一筆資料?'))location.href='update_kao_system.php?deliid=<?php echo $auto_id;?>';">
			</td>
        </tr>
