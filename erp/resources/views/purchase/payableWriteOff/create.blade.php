            ...
            <table id="master" width="100%">
                ...
                <tr>
                    ...
                    <td colspan="3">
                        ...
                        <input type="text" name="payableWriteOff[supplier_code]" class="supplier_code" value="{{ $payableWriteOff['supplier_code'] }}"  size="10">
                        <input type="text" name="payableWriteOff[supplier_name]" class="supplier_autocomplete" value="{{ $payableWriteOff['supplier_name'] }}">
                    </td>
                </tr>
                <tr>
                    <th>備註</th>
                        ...
                        <tr>
                            <td>
                                ...
                                <input class="credit_type" type="hidden"
                                    name="payableWriteOffCredit[{{ $i }}][credit_type]" value="payment">
                            </td>
                            <td>
...
@if (count($payableWriteOffDebit) > 0)
    @foreach ($payableWriteOffDebit as $i => $value)
                        <tr>
                            ...
                            <td>
                                {{ $payableWriteOffDebit[$i]['debit_date'] }}
                            ...
                            <td>{{ $payableWriteOffDebit[$i]['debit_order_amount'] }}</td>
                            <td>
                                {{ $payableWriteOffDebit[$i]['debit_order_amount'] - $payableWriteOffDebit[$i]['debit_paid_amount'] }}
                                <input type="hidden" name="payableWriteOffDebit[{{ $i }}][debit_order_amount]" value="{{ $payableWriteOffDebit[$i]['debit_order_amount'] }}">
                                <input type="hidden" name="payableWriteOffDebit[{{ $i }}][debit_paid_amount]" value="{{ $payableWriteOffDebit[$i]['debit_paid_amount'] }}">
                            </td>
                            <td>
                                <input type="text" class="debit_amount" style="text-align:right;"
