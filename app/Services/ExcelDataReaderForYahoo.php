...
class ExcelDataReaderForYahoo implements B2CExcelDataReader
{
    private $platform = 'yahoo';
...
//                'pay_status' => "付款狀態",
                'recipient' => "收件人姓名",
                'recipient_tel' => "收件人電話日", //不知道為什麼括號會被拿掉
//                'city' => "縣市",
//                'zip' => "郵遞區號",
                'address' => "收件人地址",
                'payway' => "付款別",
    ...
    protected function transformate(Collection $item)
    {
        ...
        foreach ($columnsMapping['head'] as $table => $columns) {
            ...
            foreach ($columns as $column => $title) {
                ...
                // 收件人電話
                if (
                    isset($this->columnsMapping['head']['saleOrderMaster']['recipient_tel'])
                    && $title == $this->columnsMapping['head']['saleOrderMaster']['recipient_tel']
                ) {
                    $returnData[$table][$column] = trim($item[0][$title]);
                    $returnData[$table][$column] .= ";".trim($item[0]['收件人電話夜']);
                    $returnData[$table][$column] .= ";".trim($item[0]['收件人行動電話']);
                    continue;
                }
                $returnData[$table][$column] = trim($item[0][$title]);
            }
            ...
            foreach ($item as $index => $row) {
                ...
                foreach ($columns as $column => $key) {
                    ...
                    if (
                        ...
                        if(isset($this->itemNameMapping[$row[$key]])) {
                            $process[$index][$column] = $this->itemNameMapping[trim($row[$key])];
                        } else {
                            $process[$index][$column] = trim($row[$key]);
                        }
                        continue;
                    }
                    if (
                        isset($this->columnsMapping['head']['saleOrderMaster']['price'])
                        && $key == $this->columnsMapping['head']['saleOrderMaster']['price']
                    ) {
                        $item_key = $this->columnsMapping['head']['saleOrderMaster']['quantity'];

                        $process[$index][$column] = $row[$key] / (int) $row[$item_key];
                        continue;
                    }
