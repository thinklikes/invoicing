...
class ExcelDataReaderForQDM implements B2CExcelDataReader
{
    private $platform = '';
...
//               'note' => '備註',
    ...
    public function loadData($filename) {
        try {
            ...
            if (is_null($data->pluck($this->primary)[0])) {
                throw new \Exception();
            }
        } catch (\Exception $e) {
            try {
                $data = Excel::setDelimiter("\t")->load($filename)->get();
            } catch(\Exception $e2) {
