...
class ExcelDataReaderFor91APP implements B2CExcelDataReader
{
    private $platform = '91APP';
    ...
    protected function transformate(Collection $item)
    {
        ...
        foreach ($columnsMapping['head'] as $table => $columns) {
            ...
            foreach ($columns as $column => $title) {
                ...
                // 處理溫層類別
                if (
                    isset($this->columnsMapping['head']['saleOrderMaster']['isCool'])
                    && $title == $this->columnsMapping['head']['saleOrderMaster']['isCool']
                ) {
                    $returnData[$table][$column] = strpos($item[0][$title], '常溫') !== false ? 0 : 1;
                    continue;
                }
