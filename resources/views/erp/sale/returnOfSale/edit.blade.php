...
@section('content')
        ...
        <form action="{{ url("/returnOfSale/".${$headName}['code']) }}" method="POST">
            {{ csrf_field() }}
            {{ method_field('PUT') }}
            <div id="master_table" class="custom-table">
                ...
                <div class="tr">
                    <div class="th">客戶</div>
                    <div class="td">
                        <input type="hidden" name="{{ $headName }}[company_id]" class="company_id" value="{{ ${$headName}['company_id'] }}">
                        <input type="text" name="{{ $headName }}[company_code]" class="company_code" value="{{ ${$headName}['company_code'] }}" size="5">
                        <input type="text" name="{{ $headName }}[company_name]" class="company_autocomplete" value="{{ ${$headName}['company_name'] }}" size="15">
                    </div>
                </div>
                <div class="tr">
